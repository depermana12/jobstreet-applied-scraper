...
class JobStreetScraper:
    ...
    def find_job_cards(self):
        ...
        elements = self.driver.find_elements(By.CSS_SELECTOR, "[data-automation^='job-item-']")
        if elements:
    ...
    def go_to_next_page(self):
        ...
        try:
            ...
            wait.until(EC.presence_of_all_elements_located((By.CSS_SELECTOR, "[data-automation^='job-item-']")))
    ...
    def scrape_all_jobs(self, max_pages=None,):
        ...
        while True:
            ...
            for i, card in enumerate(job_cards, 1):
                ...
                try:
                    try:
                        header_card = card.find_element(By.CSS_SELECTOR, "h4 span[role='button']")
                    except StaleElementReferenceException:
                        # Refetch all job cards and get the i-th one again
                        job_cards_refetched = self.find_job_cards()
                        header_card = job_cards_refetched[i-1].find_element(By.CSS_SELECTOR, "h4 span[role='button']")
                    
                    self.driver.execute_script("arguments[0].scrollIntoView(true);", header_card)
                    wait = WebDriverWait(self.driver, 10)
                    wait.until(EC.element_to_be_clickable(header_card))
                    header_card.click()

                    drawer = wait.until(EC.presence_of_element_located((By.CSS_SELECTOR, "[role='dialog']")))
                    raw_details_data = drawer.text.strip()

                    try:
                        link_elem = drawer.find_element(By.TAG_NAME, "a")
                        job_info['job_link'] = link_elem.get_attribute('href')
